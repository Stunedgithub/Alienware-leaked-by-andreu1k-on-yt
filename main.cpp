...
void SetupWindow()
{
	GameWnd = FindWindowA(NULL, TEXT("Steml E-Sports | Fortnite"));
...
void drawLoop(int width, int height) {
	...
	while (true) {
		...
		DWORD_PTR AActors = KmDrv->Rpm<DWORD_PTR>(Persistentlevel + Offsets::AActors);
...
void DrawESP() {
	...
	DWORD_PTR AActors = KmDrv->Rpm<DWORD_PTR>(Ulevel + Offsets::AActors);
	...
	for (unsigned long i = 0; i < entityListCopy.size(); ++i) {
		...
		localactorpos = KmDrv->Rpm<Vector2>(Rootcomp + Offsets::LocalActorPos);
...
void render() {
	...
	if (ShowMenu == true) {
		...
		ImGui::Begin("Fortnite Fn", nullptr, ImGuiWindowFlags_NoDecoration);
		{
